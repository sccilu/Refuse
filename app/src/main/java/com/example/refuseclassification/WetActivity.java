...
public class WetActivity extends BaseActivity {
    ...
    WetActivity.MyAdapter myAdapter; // 自定义适配器
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        knowledges = LitePal.where("kind = ?", "湿垃圾").find(Knowledge.class); // 查询数据库中所有 "湿垃圾" 类型的知识项
        myAdapter = new WetActivity.MyAdapter();
        recyclerView.setAdapter(myAdapter); // 设置适配器
        LinearLayoutManager manager = new LinearLayoutManager(WetActivity.this); // 设置布局管理器
